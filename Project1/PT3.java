...
class PT3{
	...
	public static final int H1 = 2; // neurons in first hidden layer
	public static final int H2 = 4; // neurons in second hidden layer
	public static final int H3 = 8; // neurons in third hidden layer
	public static final String relu = "relu";
	public static final String tanh = "tanh";
	public static final String thirdHiddenLayerActivationFunction = relu;
	...
	float[] currentError = new float[trainingSetSize*1000];
	...
	public void createModel(){
		...
		layers[3] = new Layer(H3,H2,thirdHiddenLayerActivationFunction); //or relu
		layers[4] = new Layer(K,H3,"logistic");
		...
		gradientHidden3 = new float[layers[3].getNumberOfNeurons()][layers[2].getNumberOfNeurons()];
		gradientOutputLayer = new float[K][layers[3].getNumberOfNeurons()];
		
		gradientBiasHidden1 = new float[layers[1].getNumberOfNeurons()];
		gradientBiasHidden2 = new float[layers[2].getNumberOfNeurons()];
		gradientBiasHidden3 = new float[layers[3].getNumberOfNeurons()];
	...
	public float[] forward_pass(float x[], int d, float[] y, int K){
		...
		float[] outputOfHiddenLayer3 = computeLayer(outputOfHiddenLayer2,layers[3]);
		float[] outputOfOutputLayer = computeLayer(outputOfHiddenLayer3,layers[4]);
	...
	public void backprop(float[] x, int d, float[] t, int K){
		...
		for(int i=0;i<K;i++){ 
			...
			squaredErrorOfEpoch += Math.pow(outputError[i],2);
		}
		...
		// computing the error for the third hidden layer
		float[] hidden3_delta = new float[layers[3].getNumberOfNeurons()];
		for(int i=0;i<layers[3].getNumberOfNeurons();i++){ // for each neuron in hidden layer 3
			
			for(int j=0;j<K;j++){ // for each neuron in output
				float weightFromHiddenToOutput = layers[4].getNeuron(j).getSpecificWeight(i);
				hidden3_delta[i] += outputDelta[j]*weightFromHiddenToOutput;
			}
			float hiddenNeuronOutputValue = layers[3].getNeuron(i).getOutput();
			if(layers[3].getActivationFunction().equals("tanh")){
				hidden3_delta[i] = hidden3_delta[i]*derivativeTanh(hiddenNeuronOutputValue);
			}else{ // activation function is relu
				hidden3_delta[i] = hidden3_delta[i]*derivativeRelu(hiddenNeuronOutputValue);
			}
		}
		...
		for(int i=0;i<layers[2].getNumberOfNeurons();i++){ // for each neuron in hidden layer 2
			...
			for(int j=0;j<layers[3].getNumberOfNeurons();j++){ // for each neuron in hidden layer 3
				float weightFromHiddenTwoToHiddenThree = layers[3].getNeuron(j).getSpecificWeight(i);
				hidden2_delta[i] += hidden3_delta[j]*weightFromHiddenTwoToHiddenThree;
			}
			float hidden2NeuronOutputValue = layers[2].getNeuron(i).getOutput();
			if(layers[2].getActivationFunction().equals("tanh")){
				hidden2_delta[i] = hidden2_delta[i]*derivativeTanh(hidden2NeuronOutputValue);
			}
			else{
				hidden2_delta[i] = hidden2_delta[i]*derivativeRelu(hidden2NeuronOutputValue);
			}
		}
		...
		// updating delta derivatives from hidden layer 3 to output
		for(int i=0;i<K;i++){ // iterating through the output layer
			
			float gradient = 0;
			for(int j=0;j<layers[3].getNumberOfNeurons();j++){ //iterating through the third hidden layer
				float outputOfHiddenLayer3Neuron = layers[3].getNeuron(j).getOutput();
				gradient = outputDelta[i]*outputOfHiddenLayer3Neuron;
		...
		// updating delta derivatives from hidden layer 2 to hidden layer 3
		for(int i=0; i<layers[3].getNumberOfNeurons();i++){ //iterating through the third hidden layer
			...
			for(int j=0;j<layers[2].getNumberOfNeurons();j++){ //iterating through the second hidden layer
				...
				gradient = hidden3_delta[i]*outputOfHiddenLayer2Neuron;
				gradientHidden3[i][j] = gradient;
			}
			float gradientForBias = hidden3_delta[i];
			gradientBiasHidden3[i] = gradientForBias;
		}
	...
	public void initializeWeightsOfLayers(){
		...
		// output layer
		for(int i=0; i<layers[4].getNumberOfNeurons();i++){
			Neuron neuron = layers[4].getNeuron(i);
			neuron.initializeWeightsAndBias();
		}
	}
	...
	public void gradientDescent(){
		...
		while (conditionEvaluation(epochs)){
			...
			for(int i=0; i<trainingSetSize; i++){
				...
				updateGradients(layers[3],3); //third hidden layer
				updateGradients(layers[4],4); //output layer
				if (batchIteration == B){ // a batch has been completed
					...
					updateWeightsOfLayer(layers[3]); //third hidden layer
					updateWeightsOfLayer(layers[4]);
					resetDerivativesArrays();
			...
			System.out.println("Epoch: "+epochs+" Total error: "+totalerror);// totalerror);
			epochs++;
		}
	}
	...
	private void deleteDeltasOfLayers(){
		...
		layers[4].cleanDeltasOfNeurons();
	}

	private void resetDerivativesArrays(){
		resetTempDerivatives(gradientHidden1);
		resetTempDerivatives(gradientHidden2);
		resetTempDerivatives(gradientHidden3);
		...
		resetTempDerivativesBias(gradientBiasHidden3);
	...
	private void updateGradients(Layer layer, int numberOfLayer){
		for(int i=0; i<layer.getNumberOfNeurons();i++){
			...
			for(int j=0; j<neuron.getNumberOfDerivativeDelta(); j++){
				...
				if(numberOfLayer == 3){ // third hidden layer
					neuron.setDerivativeDelta(j, neuron.getSpecificDerivativeDelta(j) + gradientHidden3[i][j]);
				}
				if(numberOfLayer == 4){ // output layer
			...
			if(numberOfLayer == 3){ // third hidden layer
				neuron.setDerivativeDeltaForBias( neuron.getDerivativeDeltaForBias() + gradientBiasHidden3[i]);
			}
			if(numberOfLayer == 4){ // output layer
	...
	public void testGeneralAbility(){
		String fileName = "PT3-" + H1 + "-" + H2 + "-" + H3 + "-" + B + "-" + layers[3].getActivationFunction() + ".txt";
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
			...
			System.out.println("total tests made  " + testMade + " and correct predictions " + correctPredictions);
